...
// TODO:
// Standarize Proposals With the MSDB Instead of Hard-Coding
const PROPOSAL1 = {
  id: 1,
  name: 'amentum',
  m: 2,
...
(async function open() {
  await node.open();
    ...
    const cosigner1 = Cosigner.fromOptions({name: 'tokensoft', token: wdb.wdb.primary.token});
    const tokensoft = await MSDB.create(OPTIONS1, cosigner1, xpub1);

    // Fix TokenDepth & Path after meeting.
    ...
    const path = await wdb.wdb.primary.getPath(recv);
  ...
  // Lets Explore the Rabbit-Hole (WIP)
